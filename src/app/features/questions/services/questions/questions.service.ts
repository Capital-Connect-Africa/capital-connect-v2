...
export class QuestionsService extends BaseHttpService<any> {
  ...
  getAllSections() {
    return this.get(`${BASE_URL}/sections`) as Observable<Section[]>
  }

  getSingleSection(id: number) {
    return this.getById(`${BASE_URL}/sections`, id) as Observable<Section>
  }
  ...
  getSingleSubsection(id: number) {
    return this.getById(`${BASE_URL}/subsections`, id) as Observable<SubSection>
  }

  getSubSectionsOfaSection(sectionId: number) {
    return this.get(`${BASE_URL}/sections/${sectionId}/subsections`) as Observable<SubSection[]>
  }
  ...
  getSingleQuestion(id: number) {
    return this.getById(`${BASE_URL}/questions`, id) as Observable<Question>
  }
  ...
  getQuestionsOfSubSection(subsectionId: number) {
    return this.get(`${BASE_URL}/subsections/${subsectionId}/questions`).pipe(map(questions => {
  ...
  getAnswersOfAQuestion(questionId: number) {
    const answers$ = this.get(`${BASE_URL}/answers`) as Observable<Answer[]>
    return answers$.pipe(map(res => res.filter(a => a.id === questionId)))
  }
  getSingleAnswer(answerId: number) {
    return this.getById(`${BASE_URL}/answers`, answerId) as Observable<Answer>
  }
